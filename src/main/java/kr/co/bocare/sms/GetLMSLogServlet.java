...
@WebServlet(urlPatterns = {"/getLMSLog"})
public class GetLMSLogServlet extends HttpServlet {
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (searchTarget != null && !searchTarget.equals("")) {
            try (PrintWriter out = response.getWriter();
                 ...
                 PreparedStatement preparedStatement = connection.prepareStatement("SELECT DISTINCT(ETC1) from MMS_LOG WHERE CALLBACK = ? AND (REQDATE  BETWEEN ? AND ? ) AND ETC2 LIKE ? GROUP BY ETC1 ORDER BY ETC1");
            ) {
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    while (resultSet.next()) {
                        ...
                        try (PreparedStatement preparedStatement2 = connection.prepareStatement("SELECT REQDATE, STATUS,  ETC2, SENTDATE,  MSG FROM MMS_LOG WHERE CALLBACK = ? AND ETC1 = ?");) {
            ...
            try (PrintWriter out = response.getWriter();
                 Connection connection = DatabaseConnector.getConnection();
                 PreparedStatement preparedStatement = connection.prepareStatement("SELECT DISTINCT(ETC1) from MMS_LOG WHERE CALLBACK = ? AND (REQDATE  BETWEEN ? AND ? ) GROUP BY ETC1 ORDER BY ETC1");
            ) {
                preparedStatement.setString(1, from);
                Date startDateDate = sqlFormatting.parse(startDate);
                preparedStatement.setDate(2, new java.sql.Date(startDateDate.getTime()));
