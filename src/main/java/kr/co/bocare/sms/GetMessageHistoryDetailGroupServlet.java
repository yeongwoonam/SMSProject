...
@WebServlet(urlPatterns = {"/getMessageHistoryDetailGroup"})
public class GetMessageHistoryDetailGroupServlet extends HttpServlet {
    private static final Logger LOGGER = LoggerFactory.getLogger(GetMessageHistoryDetailGroupServlet.class);
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (PrintWriter out = response.getWriter();
             ...
             PreparedStatement preparedStatement = connection.prepareStatement("SELECT TR_SENDDATE, TR_RSLTSTAT,  TR_PHONE, TR_REALSENDDATE,  TR_MSG, TR_ETC2  FROM sc_log WHERE TR_ETC1 = ? AND TR_ETC2 LIKE ? ORDER BY TR_REALSENDDATE desc");
        ) {
