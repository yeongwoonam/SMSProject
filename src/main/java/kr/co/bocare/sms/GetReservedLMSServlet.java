...
@WebServlet(urlPatterns = {"/getReservedLMS"})
public class GetReservedLMSServlet extends HttpServlet {
    private static final Logger LOGGER = LoggerFactory.getLogger(GetReservedLMSServlet.class);
    @Override
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        LOGGER.debug("{} {} {} {}",from,startDate,endDate,searchTarget);
        ...
        if (searchTarget != null && !searchTarget.equals("")) {
            try (PrintWriter out = response.getWriter();
                 ...
                 PreparedStatement preparedStatement = connection.prepareStatement("SELECT DISTINCT(ETC1) from MMS_MSG WHERE CALLBACK = ? AND (REQDATE BETWEEN ? AND ?) AND ETC2 LIKE ? GROUP BY ETC1 ORDER BY ETC1 ");
            ) {
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    while (resultSet.next()) {
                        ...
                        try (PreparedStatement preparedStatement2 = connection.prepareStatement("SELECT REQDATE,  ETC2,  MSG FROM MMS_MSG WHERE CALLBACK = ? AND ETC1 = ?")) {
            ...
            try (PrintWriter out = response.getWriter();
                 Connection connection = DatabaseConnector.getConnection();
                 PreparedStatement preparedStatement = connection.prepareStatement("SELECT DISTINCT(ETC1) from MMS_MSG WHERE CALLBACK = ? AND (REQDATE BETWEEN ? AND ?) GROUP BY ETC1 ORDER BY ETC1");
            ) {
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    while (resultSet.next()) {
                        ...
                        JSONObject jsonObject = new JSONObject();
                        jsonObject.put("trEtc", trEtc1);
                        System.out.println("trEtc : "+trEtc1);
                        try (PreparedStatement preparedStatement2 = connection.prepareStatement("SELECT REQDATE,  ETC2,  MSG FROM MMS_MSG WHERE CALLBACK = ? AND ETC1 = ?")) {
                            preparedStatement2.setString(1, from);
                            preparedStatement2.setString(2, trEtc1);
                            boolean isFirst = true;
